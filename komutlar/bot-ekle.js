...
if(db.fetch(`realbots_${kelimeilk}_${message.guild.id}`)) return message.channel.send("Botunuz zaten başvuru listesinde yer alıyor.")
  

        db.set(`realbots_${kelimeilk}_${message.guild.id}`, message.author.id)
...
message.channel.send(evomeddd)
   ...
   db.delete(`realbots_${kelimeilk}_${message.guild.id}`)
